    ...
    window.addEventListener(
        ...
        async (event: EIP6963AnnounceProviderEvent) => {
            ...
            if (accounts.length > 0) { // TODO: testing 1 account for now
                setProviderAndAddress(provider, accounts[0]);
...
async function displayEthBalance(address: string, provider: Readonly<EIP1193Provider>): Promise<void> {
    try {
        const balanceInHex = await provider.request({ method: "eth_getBalance", params: [address, "latest"] }) as string; // in wei
        const balanceEth: string = ethers.formatEther(balanceInHex);
